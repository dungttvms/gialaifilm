...
import { getPhimChieuRap } from "./movieSlice.js";
...
function PhimChieuRap() {
  ...
  const movies = useSelector((state) => state.movie.Phim_Chieu_Rap);

  useEffect(() => {
    dispatch(getPhimChieuRap({ page: page + 1, limit: NUMBER_OF_LIMIT }));
  }, [page, dispatch]);
  ...
  return (
    <Container sx={{ mt: 2 }} ref={scrollRef}>
      <Helmet>
        <title>Phim Chiếu Rạp | Phim Gia Lai</title>
      </Helmet>
      {movies.length > 0 ? (
        <>
          <Box
          ...
          >
            ...
            >
              CÓ {fNumber(totalMovies)} PHIM CHIẾU RẠP ĐƯỢC TÌM THẤY
            </Typography>{" "}
            <Pagination
