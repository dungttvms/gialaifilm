...
import { getPhimBo } from "./movieSlice.js";
...
function PhimBo() {
  ...
  const movies = useSelector((state) => state.movie.Phim_Bo);

  useEffect(() => {
    dispatch(getPhimBo({ page: page + 1, limit: NUMBER_OF_LIMIT }));
  }, [page, dispatch]);
      ...
      {movies.length > 0 ? (
        <>
          ...
          >
            ...
            >
              CÓ {fNumber(totalMovies)} PHIM BỘ ĐƯỢC TÌM THẤY
            </Typography>{" "}
            <Pagination
