...
function PhimHoatHinh() {
  ...
  const movies = useSelector((state) => state.movie.Phim_Hoat_Hinh);

  useEffect(() => {
    dispatch(getPhimHoatHinh({ page: page + 1, limit: NUMBER_OF_LIMIT }));
  }, [page, dispatch]);
      ...
      {movies.length > 0 ? (
        <>
          ...
          >
            ...
            >
              CÓ {fNumber(totalMovies)} PHIM HOẠT HÌNH ĐƯỢC TÌM THẤY
            </Typography>{" "}
            <Pagination
