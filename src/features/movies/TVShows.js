...
function TVShows() {
  ...
  const movies = useSelector((state) => state.movie.TV_Shows);

  useEffect(() => {
    dispatch(getTVShows({ page: page + 1, limit: NUMBER_OF_LIMIT }));
  }, [page, dispatch]);
      ...
      {movies.length > 0 ? (
        <>
          ...
          >
            ...
            >
              CÓ {fNumber(totalMovies)} CHƯƠNG TRÌNH GIẢI TRÍ ĐƯỢC TÌM THẤY
            </Typography>{" "}
            <Pagination
